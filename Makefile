...
bundle: bot
	...
	jar cfm "$(DIST)" "$(MANIFEST)" -C "$(BINDIR)" . "license.txt" "$(IMGDIR)"/*.png
...
docs:
	...
	javadoc -d "$(DOCSDIR)" -version -author -windowtitle "RSBot API Documentation" -header "RSBot&trade; API" -footer "`cat $(DOCSCFG)/footer.txt`" -bottom "`cat $(DOCSCFG)/bottom.txt`" -charset "utf-8" -docencoding "utf-8" -classpath src `cat $(DOCSCFG)/packages.txt`
