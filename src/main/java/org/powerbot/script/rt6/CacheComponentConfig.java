...
public class CacheComponentConfig {
	...
	private CacheComponentConfig(final Block.Sector sector, final int id) {
		this.componentId = id;
		this.stream = new JagexStream(sector.getPayload());
		read();
	}

	private CacheComponentConfig() {
		this.componentId = -1;
		this.stream = null;
	}

	static CacheComponentConfig load(final int id) {
		return load(Bot.CACHE_WORKER, id);
	}

	private static CacheComponentConfig load(final CacheWorker worker, final int id) {
		final Block b = worker.getBlock(3, id >>> 16); //archive
		if (b == null) {
			return new CacheComponentConfig();
		}
		final Block.Sector s = b.getSector(id & 0xffff); //file data
		if (s == null) {
			return new CacheComponentConfig();
		}
		return new CacheComponentConfig(s, id);
	}
	...
	Object[] method4273(JagexStream stream) {
	...
	private int[] method4250(JagexStream stream) {
