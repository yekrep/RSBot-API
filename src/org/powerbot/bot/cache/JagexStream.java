...
public class JagexStream {
	...
	protected final void assertLeft(final int num) {
		if (!checkLeft(num)) {
			throw new BufferUnderflowException();
		}
	}

	public final boolean checkLeft(final int num) {
	...
	protected final int getSmart(final int smallSize, final int bigSize, final boolean signed) {
		final int num = getUByte();
		final int size = ((num & BYTE_SIGN_BIT) == BYTE_SIGN_BIT ? bigSize : smallSize);
		return joinSmart(num, size, signed);
	}

	protected final int joinSmart(final int first, final int size, final boolean signed) {
