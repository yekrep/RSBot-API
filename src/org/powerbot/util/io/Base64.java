...
public class Base64 {
	...
	public static byte[] encode(byte[] bytes) {
		int inputLength = bytes != null ? bytes.length : 0;
		if (inputLength == 0) {
			return new byte[0];
		}
		int evenLength = (inputLength / 3) * 3;
		int charLength = ((inputLength - 1) / 3 + 1) << 2;
		int outputLength = charLength + ((charLength - 1) / 76 << 1);
		byte[] encodedBytes = new byte[outputLength];
		for (int decodedIndex = 0, encodedIndex = 0, entrySize = 0; decodedIndex < evenLength; ) {
			int assembledInteger = (bytes[decodedIndex++] & 0xff) << 16 | (bytes[decodedIndex++] & 0xff) << 8 | (bytes[decodedIndex++] & 0xff);
		...
		int padding = inputLength - evenLength;
		if (padding > 0) {
			int assembledInteger = ((bytes[evenLength] & 0xff) << 10) | (padding == 2 ? ((bytes[inputLength - 1] & 0xff) << 2) : 0);
	...
	public static byte[] decode(byte[] encodedBytes) {
		...
		for (int encodeIndex = 0, decodeIndex = 0; decodeIndex < outputLength; ) {
			...
			for (int validIndex = 0; validIndex < 4; validIndex++) {
				int c = INT_ARRAY[encodedBytes[encodeIndex++] & 0xff];
				if (c >= 0) {
